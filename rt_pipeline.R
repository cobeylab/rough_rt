...
full_rt_pipeline <- function(df, ## Data frame containing time series of observations
  ...
  ## 3. Use Bayesian RL to infer times of infection given the posterior parms of the delay
  deconvolved <- deconvolve(df$obs, times = df$time, delay_posterior = delay_pars, nboot = nboot, delay_type = delay_type) 
  
  
  
  ## LAUREN - KILLING THE UPSCALING FOR NOW SINCE THE DATAFRAME FROM SAMPLING ISN'T COMPATIBLE WITH THE UPSCALING FUNCTION
  ## Produces n_replicates of upsampled versions from each deconvolved observation vector
  ## 4. Upscale from deconvolved times of infection to total infections based on p_obs
  deconvolved <- lapply(deconvolved, FUN = function(df.in) { #all columns for deconvolved
    est_total_inf(df.in, 
                  p_obs = p_obs, 
                  obs_colname = "samples", #'RL_result,
                  n_replicates = 1) 
  }
  )
  ...
  ## Uses Cori to compute Rt from each infection curve
  ...
  rt_ests$summary <- rt_ests$summary %>%
    merge(dplyr::select(df, date, time), by = 'time')
  ...
  deconvolved <- deconvolved %>%
    bind_rows(.id = 'rep') %>%
    merge(dplyr::select(df, date, time), by = 'time') %>%
    dplyr::select(date, rep, RL_result, infections.1) %>%
    setNames(c('date', 'rep', 'obs_infections_est', 'total_infections_est')) 
  
  RL_plot <- bind_rows(
    deconvolved %>% dplyr::select(-total_infections_est) %>% pivot_longer(3),
    df %>% mutate(rep = NA, name = obscolname) %>% rename(value = obs) %>% dplyr::select(date, rep, name, value)
  )%>% 
    ggplot()+
  ...
  upscale_plot <- bind_rows(
    deconvolved %>% dplyr::select(-obs_infections_est) %>% pivot_longer(3),
    df %>% mutate(rep = NA, name = obscolname) %>% rename(value = obs) %>% dplyr::select(date, rep, name, value)
  )%>% 
    ggplot()+
    geom_line(aes(x = date, y = value, group = rep, color = name), alpha = .8) +
