est_total_inf <- function(df, ## Data frame containing multiple columns with daily case counts (output from Richardson_Lucy)
                          ...
                          obs_colname = "samples", ## Name of the column holding the observations is obs_colname.x, where x = 1, 2, ... depending on number of samples per Richardson_Lucy run
                          n_replicates = 1) ## Number of times to resample total infections 
{
  ...
  #print(dim(df %>% dplyr::select(starts_with("samples"))))
  
  print(obs_colname)
  
  obs = df %>% dplyr::select(-time)  #%>% unlist(use.names = F) #*********************** me: select(starts_with(samples.*)) since that's the format of the RL dataframe output
  
  obs = ifelse(is.na(obs), 0, obs)
  
  print(obs)
  
# obs is: a vector containing all samples in a row (414 x 1) instead of keeping the dimensions of df (138 x 3)
  
  ## For each entry in the observations column, upscale
  infection_samples = replicate(n = n_replicates, draw_one_sample(obs, p_obs)) #produce n_replicates samples upscaled by neg binom
  
  print(names(infection_samples)) #infection_samples is a vector - doesn't have names
