...
del_hosp_to_report <- runif(n = length(del_onset_to_hosp_hk), .5, 1)

delay_vec_hk <- as.numeric(del_inf_to_onset + del_onset_to_hosp_hk + del_hosp_to_report)
delay_vec_ni <- as.numeric(del_inf_to_onset + del_onset_to_hosp_Nishiura + del_hosp_to_report)
```
## Fit gamma distribution
```{r}
sprintf('mean delay from infection to hospital reporting in hk is %.2f d', mean(delay_vec_hk))
sprintf('mean delay from infection to hospital reporting based on Linton et al is %.2f d', mean(delay_vec_ni))
```
...
bind_rows(list(
  ...
  ggtitle('Linton (living): Delay from infection to hospitalization', subtitle = sprintf('6.1d incubation; %.2fd onset to hosp; 1.25 reporting\nmean=%.2f, var=%.2f\nlognormal mu = %.2f, sigma=%.2f',
...
bind_rows(list(
  ...
  geom_line(aes(x = wait, y = med, color = dist), lwd = 1) +
  geom_ribbon(aes(x = wait, ymin = lower, ymax = upper, fill = dist), alpha = .5)+
  ggtitle('HK: delay from infection to hospitalization', subtitle = sprintf('6.1d incubation; %.2fd onset to hosp; 1.25 reporting\nmean=%.2f, var=%.2f\nlognormal mu = %.2f, sigma=%.2f',
          mean(del_onset_to_hosp_hk), mean(delay_vec_hk), var(delay_vec_hk), mean(params_lognormal_hk$mu), mean(params_lognormal_hk$sigma)))
```

## Save the posterior
```{r}
write_rds(params_lognormal_hk, 'delay_infection_to_hosp_report_posterior.rds')
```
