...
## Plot against data
```{r}
wait <- data.frame(wait = floor(delay_vec_ni)) %>%
  group_by(wait) %>%
  summarise(count = n()) %>%
   mutate(kind = 'ni')
...
wait %>%
  ggplot() +
  geom_bar(aes(x = wait, y = count), stat = 'identity')+
  facet_grid(.~kind)
...
bind_rows(list(
  ...
  ggtitle('Delay from infection to testing', subtitle = sprintf('Empirical mean=%.2f, var=%.2f\nlognormal mu = %.2f, sigma=%.2f',
          mean(delay_vec_ni), var(delay_vec_ni), mean(params_lognormal_ni$mu), mean(params_lognormal_ni$sigma)))
```
...
bind_rows(list(
  ...
  geom_ribbon(aes(x = wait, ymin = lower, ymax = upper, fill = dist), alpha = .5)+
  ggtitle('Delay from infection to testing', subtitle = sprintf('Empirical mean=%.2f, var=%.2f\nlognormal mu = %.2f, sigma=%.2f',
          mean(delay_vec_hk), var(delay_vec_hk), mean(params_lognormal_hk$mu), mean(params_lognormal_hk$sigma)))
```
