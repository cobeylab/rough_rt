...
get_RL <- function(observed, ## Vector of observed cases. Let L = length(observed)
  ...
  lambda <- lambda0
  curr_u <- true_dat 
  all_u <- matrix(NA, nrow = max_N, ncol = length(true_dat)) #samples of the $p(\vec{u}\vert \vec{b})$ distribution (with $\lambda$)
  all_lambda <- matrix(NA, nrow = max_N, ncol = 1) #samples the $\lambda$ distribution.
