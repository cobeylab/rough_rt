...
summarise_all_estimates <- function(
  ...
  write_csv(fulldf, path = sprintf('../figs/cli_nadmit_%s/full_outupts_epinow2_%s.csv', tooday, dt), append = T)
  write_csv(Rtdf, path = sprintf('../figs/cli_nadmit_%s/Rt_epinow2_%s.csv', tooday, dt), append = T)
